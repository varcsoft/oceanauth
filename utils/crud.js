...
const getbyevent = async (req, res, next, object, include,where) => {
    try {
        where=where?where:{};
        const data = await object.findMany({ where: { event_id: Number(req.params.id),...where }, include });
        return sendresponse(res, data, 200,req);
    } catch (e) {
        next(e);
    }
...
const update = async (req, res, next, object, required, include) => {
    try {
        ...
        throw new ApiError(403,"Error while updating");
    }
}

export default { get, getbyid, getbyevent, deletebyid, create, update };